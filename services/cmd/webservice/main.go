...
import (
	...
	"github.com/your-repo/lyn-backend/internal/webservice"
...
func main() {
	// Initialize logger
	logger := logging.NewLogger("webservice")
	logger.Info("Starting Web Service")

	// Load configuration
	cfg, err := config.Load("webservice")
	if err != nil {
		logger.Fatal("Failed to load configuration", "error", err)
	}

	// Initialize server
	server, err := webservice.NewServer(cfg, logger)
	if err != nil {
